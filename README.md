...
- 月牙画小了一半有余。相机是 42° 透视、架在 3.05 个半径外，这么近看球会挡掉自身边缘
  18° 的一圈，剩下的还被挤向圆盘边——而月牙全是边缘。面板报 11.9% 照亮时，渲染只有
  4.0% 受光，亮边宽度 9.2% 半径，应为 23.8%。月球从地球看只有 0.52°，本该是正交投影；
  新增 `gx::ortho`，视点同时远移，使逐片元视线方向不再在圆盘上散开。现在各相位的受光
  比例都落在 Meeus 的 k 上。
- 光源原先在 Y 上固定加 18（平面内分量为 ±100），把太阳抬出黄道面约 10°。这既让受光
  比例偏离面板上印的百分比（角距 140° 时渲染 96.2%，应为 88.3%），也让明暗界线偏离
  亮边方位角。现为 0。
- Lambert 余弦不是月壤的反射律。尘埃强烈背向散射，所以月球边缘几乎与圆面中心一样亮
  ——照片里的月牙是一道均匀亮弧，满月是一枚平盘而非有明暗的球。新增 `uLunar` 启用
  lunar-Lambert 定律（McEwen 1991），仅用于月球。
- 新增 `uExposure`：月壤反照率约 12%，按真实反照率画在夜空上近乎全黑，而暗适应的眼睛
  与对着月亮拍照的相机都会为月亮曝光。月相面板取 2.5，日月行星同框的场景保持 1；
  可读性用的环境光留在曝光之外。

**3D 月相的取向**

- 此前显示的是**月球背面**，而且是侧躺着的：月相面板构建模型矩阵时漏了
  `meshAxisFixFor("moon")`，用的是 OBJ 原始坐标系，其极轴离月球真实极轴 79°。
- 该矩阵本身的子午线相位还差 48°。月球贴图不是等距圆柱投影（是 cube net 图集），
  按顶点做最小二乘只能定出极轴。改用另一种量法：通过 mesh 自身 UV 采样贴图，把月海
  暗区在球面上聚类，再拿孤立的几块对月面坐标——危海（17.0°N 59.1°E）与莫斯科海
  （27.3°N 147.9°E）都偏东 48.3°，史密斯海偏 43.8°（位于边缘，采样被压缩）。扣除
  48.3° 后三者归位，纬度误差在 3° 以内。太阳系视图的月球取向一并纠正：它的天平动模型
  解的正是「+Z 落在本初子午线」，这个前提此前并不成立。
- 「真实取向」现在也转动月球本体，而不只是转光照：按 Meeus 第 53 章算出天平动（月面
  经纬）与月轴方位角 P，把当时真正朝向地球的那一面转到镜头前，并把月轴摆到它在天上的
  角度。月球只是"平均"以同一面朝地球——轨道偏心而自转匀速，赤道又对轨道有倾角，
  于是天平动在经度上约 ±8°、纬度上约 ±7°，一个月里能让约 59% 而非 50% 的月面转入视野。
  面板同时列出天平动经纬与月轴方位角。
- 3D 月相右下角加「还原视角」按钮：拖动转过之后可以一键转回。触摸端按钮更大，视角
  已在原位时半透明显示；从按钮上起手拖动不会把月球转走。

**回归测试**

- `test/moonphase_test.cpp` 补齐了这些本该拦住上述问题的检查：3D 受光面积对 Meeus 的
  k（各相位）、旧透视相机确实丢掉三分之二月牙、月海位置对公布的月面坐标（6° 内）、
  月球坐标系矩阵是纯旋转、满月边缘平坦度（0.95 半径处不低于中心的 90%，旁列 Lambert
  的 0.31 作对照）、Meeus 例 53.a 的天平动与 P（0.02° 内）、天平动一年包络、以及取向
  矩阵合成的 36 组组合。
- `gui/mesh_frames.h`：各网格的坐标系校正矩阵移到不依赖 GL 的头文件，测试才能真正校验
  它们——一个没人能校验的实测常数，正是月球方位错了 48° 还能一直留着的原因。
